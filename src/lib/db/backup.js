...
export async function backupDbLite(adapter, destDir, destName = "data.sqlite") {
  // ATTACH DATABASE is SQLite-only, and a managed Postgres is backed up by the
  // provider (Neon keeps point-in-time history), so there is nothing useful to
  // do here. Returning null keeps the caller's "backup failed, continuing"
  // path from firing on every schema bump.
  if (adapter.dialect === "postgres") return null;
