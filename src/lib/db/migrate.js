...
async function importWithAssertion(adapter, tableName, rows, insertFn, rowMeta) {
  ...
  // Number(): Postgres returns int8 (COUNT(*)) as a string unless a type parser
  // is installed. The pg adapter installs one, but the strict !== below is not
  // worth betting a migration abort on.
...
async function runVersionedMigrations(adapter) {
  ...
  await adapter.exec(buildCreateTableSql("_meta", TABLES._meta, adapter.dialect));
...
async function syncSchemaFromTables(adapter) {
  for (const [tableName, def] of Object.entries(TABLES)) {
    ...
    await adapter.exec(buildCreateTableSql(tableName, def, adapter.dialect));

    // Diff columns. PRAGMA is SQLite-only; information_schema is the standard
    // catalog. Postgres folded the DDL identifiers to lower case, so compare
    // case-insensitively on both engines.
    const existingNames = new Set(
      adapter.dialect === "postgres"
        ...
        : (await adapter.all(`PRAGMA table_info(${tableName})`)).map((r) => String(r.name).toLowerCase()),
    );
    for (const [colName, colDef] of Object.entries(def.columns)) {
      if (!existingNames.has(colName.toLowerCase())) {
        ...
        const safeDef = colDef
          ...
          .replace(/UNIQUE/i, "")
          .trim();
        // Postgres supports IF NOT EXISTS here; SQLite does not, so the guard
        // stays the existingNames check above plus the try/catch below.
        try {
...
export async function runMigrationOnce(adapter) {
  ...
  // Postgres has no local file to snapshot — the provider owns backups there.
  const schemaChanging = !fresh && storedSchemaVer < SCHEMA_VERSION && adapter.dialect !== "postgres";
  if (schemaChanging) {
    try {
  ...
  //
  // The marker is a local file, so it only speaks for the local SQLite file it
  // was written beside. On Postgres the DB's own emptiness (`fresh`) is the
  // authority — which also means pointing a fresh Postgres at a machine that
  // still has the legacy JSON imports it, rather than silently starting empty.
  const alreadyImported = adapter.dialect === "postgres" ? false : fs.existsSync(MIGRATED_MARKER);
  ...
  if (fresh && hasLegacy && !alreadyImported) {
    ...
    pruneOldBackups();
    console.log(`[DB][migrate] JSON → ${adapter.dialect} in ${Date.now() - t0}ms | legacy JSON kept at DATA_DIR | backup: ${backupDir}`);
    return;
  }
